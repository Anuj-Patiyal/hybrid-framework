...
get_next_version() {
    ...
    if $IS_INITIAL_RELEASE; then
        NEXT_VERSION="v0.1.0"
        log_info "Initial release - setting next version to: $NEXT_VERSION"
    else
        NEXT_VERSION="v${major}.$((minor+1)).0"
        log_info "Standard release - setting next version to: $NEXT_VERSION"
    fi
}
...
compute_release_versions() {
    ...
    if $IS_INITIAL_RELEASE; then
        ...
        # Compute proper next development version for initial release
        local initial_v="${release_tag#v}"
        IFS='.' read -r initial_maj initial_min initial_patch <<< "$initial_v"
        dev_next="v${initial_maj}.$((initial_min + 1)).0"

        log_info "Initial release - Release tag: $release_tag, Next development version: $dev_next"
    else
        ...
        log_info "Standard release - Release tag: $release_tag, Next development version: $dev_next"
    fi

    echo "$release_tag $dev_next"
}
...
- **Next Development Version**: \`${next_dev}\`
- **Version Increment**: Validated and approved
...
process_labels() {
    ...
    for label in $desired_labels; do
        ...
        if [[ ",${existing_labels}," == *",${label},"* ]]; then
            log_skip "Release label '$label' already exists on PR"
            continue
        fi
        ...
        if [[ ",${current_repo_labels}," != *",${label},"* ]]; then
            ...
            if ! gh label create "$label" --color "$LABEL_COLOR" --description "Release automation" 2>/dev/null; then
                log_warn "Label '$label' may already exist or creation failed - continuing..."
            else
        ...
        # Add label to PR
        log_info "Adding release label '$label' to PR"
        if gh pr edit "$pr_num" --add-label "$label" >/dev/null 2>&1; then
            log_success "Added release label '$label'"
        else
            log_warn "Failed to add label '$label' (may already be applied or no permissions)"
        fi
    done
}
...
main() {
    ...
    # Compute release versions dynamically
    local version_data
    version_data=$(compute_release_versions)
    local release_tag=$(echo "$version_data" | awk '{print $1}')
    local dev_next=$(echo "$version_data" | awk '{print $2}')

    log_info "Computed versions - Release: $release_tag, Next Development: $dev_next"
